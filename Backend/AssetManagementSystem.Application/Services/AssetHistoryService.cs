...
{
    public async Task<IEnumerable<AssetHistoryResponseDto>> GetAssetHistoryAsync(
        Guid assetId,
        CancellationToken cancellationToken)
    {
        if (!await assetRepository.ExistsAsync(assetId, cancellationToken))
            throw new NotFoundException("Asset not found.");

        var history = await assetHistoryRepository.GetAssetHistoryAsync(assetId, cancellationToken);
